...
; avoid anything nearby that is not black
    set obstacles patches in-cone vision 60 with [pcolor != black]
    set obstacles other turtles with [color != [color] of myself] in-radius vision
    if (any? obstacles)
         [flee]
end

to flee
  let obstacles-in-front obstacles in-cone 3 30
