...
to setup
  clear-all
  crt population
    [ ifelse random 2 > 0.5 [ set color blue ] [ set color cyan ]  ;; random shades look nice
...
to go
  ...
  repeat 1 [ ask turtles [ fd speed ] display ]
...
to flock  ;; turtle procedure
  ...
  ;turn-away (first calculVector) (first but-first calculVector)
  if (first calculVector)  + (first but-first calculVector) != 0 
  [ turn-towards angleFromVect calculVector max-align-turn ]
end
...
to-report calculVector
  let aA multiplyScalarvect max-align-turn align
  let eE multiplyScalarvect max-separate-turn separate
  let cC multiplyScalarvect max-cohere-turn cohere
  report additionvect aA (additionvect eE cC)
end

;;; SEPARATE

to-report separate  ;; turtle procedure
   ifelse ( any? flockmates) and (distance nearest-neighbor < minimum-separation)
  [ report vectFromAngle (towards nearest-neighbor + 180) (1) ]
  [ report vectFromAngle 0 0]
end

;;; ALIGN

to-report align  ;; turtle procedure
  ifelse any? flockmates
  [ report average-flockmate-heading ]
  [ report list 0 0]
end
...
to-report cohere  ;; turtle procedure
  ifelse any? flockmates
  [ report average-heading-towards-flockmates ]
  [ report list 0 0]
end
