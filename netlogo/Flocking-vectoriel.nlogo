...
to setup
  ...
  draw-obstacles
  crt population
    [ ifelse random 2 > 0.5 [ set color blue ] [ set color cyan ]
      ;set color yellow - 2 + random 2
...
to go
  ...
  repeat 2 [ ask turtles [ fd 0.5 ] display ]
...
to draw-obstacles
  ...
  while [i > 0] [
    let j size-obs
    while [j > 0] [
      ask patch (i - tare) (j - tare) [set pcolor yellow]
      set j j - 1
    ]
    set i i - 1
  ]
...
to flock  ;; turtle procedure
    find-flockmates
    let v calculVector
    if (first calculVector)  + (first but-first calculVector) != 0 
    [
      ;show calculVector
      turn-towards angleFromVect v max-align-turn
    ]
end

to-report calculVector
  let aA multiplyScalarvect factor-separate vectAlign
  let eE multiplyScalarvect factor-align vectSeparate
  let cC multiplyScalarvect factor-cohere vectCohere
  let dD multiplyScalarvect factor-avoid vectAvoid
  let eD multiplyScalarvect factor-goal vectGoal
  ;report eD
  report additionvect aA (additionvect eE (additionvect cC (additionvect dD eD)))
end
...
to-report vectGoal
  let goal patch 0 0
  let x-component sin (towards goal)
  let y-component cos (towards goal)
...
to-report vectSeparate
  if (not (any? flockmates)) [ report list 0 0 ]
  let vs 0
...
to-report vectAvoid
  let obstacle min-one-of (patches in-cone vision 30 with [pcolor = red]) [distance myself]
  
  if (obstacle = nobody) [
     set obstacle min-one-of (other turtles in-cone vision 60 with [color != [color] of myself]) [distance myself]
  ]
  
  if (obstacle = nobody) [ report list 0 0 ]
  let x-component sin (towards obstacle + 180)
  let y-component cos (towards obstacle + 180)
  report list x-component y-component
end
